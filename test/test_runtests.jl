using Test
using Petri
using pflow: Pflow, place!, arc!, guard!, transition!, to_state, to_json, to_svg, to_html, to_model, default_rates, set_rates
...
@testset "Pflow Model Tests" begin
    ...
    @testset "Petri Net Model" begin
        ...
        @test initial_state[:item0] == 1
        @test initial_state[:item1] == 1
        @test initial_state[:item2] == 1
        @test initial_state[:item3] == 1
        @test initial_state[:weight] == 0
        @test initial_state[:value] == 0
        @test initial_state[:capacity] == 15
